...
このAWSアカウントには SonicMoov が運営する**2つの独立したサービス**が同一インフラ上に同居している。

| サービス名 | 種別 | 主なリソース | 稼働状況 |
|---|---|---|---|
| **GUILE** | インタースティシャル広告配信（静止画・HTML、フルスクリーン型）。`is-elb` の "is" は Interstitial の略 | guile-interstitial, is-elb, cdn-guile, RDS(guile) など | ✅ 稼働中 |
| **admovie** | 動画広告配信。GUILE の動画広告機能。コンテンツは `cdn.jswfplayer.jp/admovie/` で配信（JswfPlayer の CDN を間借り） | admovie-elb01/02, admovie-sonicmoov-elb01/02, ElastiCache(admovie), c3.large×2台 | ✅ **稼働中**（週間リクエスト約2,500万件・2026-05-18 実測） |
| **JswfPlayer** | Flash（SWF）コンテンツを JavaScript で動作させるプレイヤー配信 SaaS。**GUILE とは独立したサービス**。契約管理・課金・顧客管理を含む | cdn.jswfplayer.jp（S3 15GB）、CloudFormation `jswf` スタック（2014年〜）、jswfplayer-api/admin/dg-admin | ⚠️ 稼働状況不明（要確認） |

### サービス間の関係（ソースコード・インフラから確認）

```
【JswfPlayer】（2014年〜の古いサービス）
  jswfplayer-api     : SWF→JS プレイヤー配信 API（PHP）
  jswfplayer-admin   : 管理画面（Symfony 1）
  jswfplayer-dg-admin: 契約者向け管理・課金（Symfony 1）
  jswfplayer-wordpress: 公開サイト
  cdn.jswfplayer.jp  : S3 コンテンツ配信（15GB）
       ↑
       | CloudFormation `jswf` スタックを構築（スタック名は JswfPlayer 由来）
       | GUILE が後からこのインフラに相乗り
       ↓
【GUILE + admovie】（JswfPlayer のインフラ上に構築）
  guile-interstitial  : インタースティシャル広告配信
  admovie-server01/02 : 動画広告配信（cdn.jswfplayer.jp/admovie/ を間借り）
  guile-deploy-config : roles/jswf-admovie として admovie をデプロイ
```

**根拠（ソースコードより）:**
- `guile-frontend-microad/Gruntfile.js`: 本番配信パスが `//cdn.jswfplayer.jp/admovie/` と明記
- `guile-deploy-config/roles/jswf-admovie`: admovie サーバーのプロビジョニングロール名に `jswf` が含まれる
- `guile-deploy-config/jswf-admovie.prd.json`: Packer AMI 名が `jswf-admovie-{{timestamp}}`

### ⚠️ jswf廃止前にGUILE側で必要な移行作業

JswfPlayer は廃止予定だが、**admovie は GUILE の主要機能のため廃止しない**。  
GUILE は JswfPlayer のインフラに3層で依存しており、**jswf 廃止前に GUILE 側で移行を完了させる必要がある**。

#### 依存レイヤーと必要な移行作業

| # | 依存内容 | jswf廃止で起きること | GUILE側の対応（jswf廃止前に必須） |
|---|---|---|---|
| ① | **VPC/ネットワーク**（`jswf` CloudFormation スタック） | スタック削除で EC2・RDS・ElastiCache が全停止 | **GUILE リソースを新 VPC/スタックへ移行**、または jswf スタックから切り離す |
| ② | **CDN**（`cdn.jswfplayer.jp` S3 バケット）| バケット削除で admovie 広告が全停止 | **admovie コンテンツを `cdn-guile` 等に移行**し、guile-frontend-* の配信URL を変更 |
| ③ | **admovie API**（`admovie.jswfplayer.jp`） | API 停止で admovie トラッキング不能 | **admovie サーバーを独自ドメインで運用**するよう変更 |
...
Step 1: admovie CDN を移行
  - cdn.jswfplayer.jp/admovie/ の内容を cdn-guile（または新バケット）へコピー
  - guile-frontend-* の Gruntfile.js 配信パスを新 CDN URL へ変更してデプロイ

Step 2: admovie API ドメインを移行
  - admovie.jswfplayer.jp → admovie.guile.jp 等の独自ドメインに切り替え
  - 広告テンプレート内の cc.php / mc.php エンドポイントを更新

Step 3: GUILE インフラを jswf スタックから独立
  - GUILE 専用の VPC / CloudFormation スタックを新規作成
  - EC2・RDS・ELB 等をリソースごと移行（Blue/Green 方式が安全）

【移行完了後】

Step 4: jswf 側リソースの削除
  - JswfPlayer アプリサーバー終了
  - cdn.jswfplayer.jp バケット削除
  - jswf CloudFormation スタック削除
```

> ⚠️ この移行は**コスト削減ではなく、jswf廃止を安全に進めるための前提条件**。  
> 移行完了前に jswf スタックを削除すると GUILE・admovie が即時停止するため、廃止スケジュールの調整が必要。
...
| スタック名 | ステータス | 作成日 | 用途 |
|---|---|---|---|
| `jswf` | CREATE_COMPLETE | 2014-03-03 | **GUILE / JSWF Player 基盤ネットワーク全体を管理**（VPC・IGW・ルートテーブル・SG・NAT-PRX01/02 等 100リソース）。スタック名 `jswf` は JSWF Player サービス（Flash→JS変換プレイヤー）に由来すると推測 |
