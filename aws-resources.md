...
### ⚠️ JswfPlayer 廃止時の GUILE への影響

JswfPlayer は廃止予定。GUILE は JswfPlayer のインフラに以下の3層で依存しているため、**廃止順序を誤るとGUILEが停止する**。

#### 依存レイヤー

| 依存内容 | 影響範囲 | JswfPlayer廃止で壊れるか |
|---|---|---|
| **① VPC / ネットワーク**（`jswf` CloudFormation スタック） | GUILE の EC2・RDS・ElastiCache すべて | ❌ **スタック削除で全サービス停止** |
| **② CDN**（`cdn.jswfplayer.jp` S3バケット） | admovie 動画広告の配信（JS・SWF・素材） | ❌ **バケット削除で admovie 広告が全停止** |
| **③ admovie API**（`admovie.jswfplayer.jp/cc.php`, `/mc.php`） | 動画広告のクリック・再生カウント計測 | ❌ **API停止で admovie トラッキング不能** |

#### 廃止の安全な順序

```
1. admovie の停止確認（クライアントへの事前告知）
   → admovie-server01/02 終了、admovie 系 ELB 削除
   → ElastiCache(admovie) 削除

2. cdn.jswfplayer.jp の admovie コンテンツ整理
   → S3バケット内の /admovie/ 以下を削除 or バケットごと削除

3. GUILE リソースを jswf スタックから分離（または GUILE も同時廃止）
   → GUILE も廃止する場合: EC2・RDS・ELB 等を先に削除してからスタック削除
   → GUILE を継続する場合: 新しい VPC/スタックへ移行してから jswf スタック削除

4. jswf CloudFormation スタック削除
   → GUILE リソースがゼロになってから実施
```

> ⚠️ `jswf` スタックを先に削除すると GUILE が即時停止する。必ず GUILE 側のリソースをすべて終了・移行してから削除すること。
