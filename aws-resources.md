...
| **JSWF Player** | Flash（SWF）コンテンツを JavaScript で動作させるプレイヤーサービス。`jswf` という名称は JavaScript + SWF の略と推測 | cdn.jswfplayer.jp（S3 15GB）、CloudFormation `jswf` スタック（2014年〜）| ⚠️ 稼働状況不明（要確認） |
