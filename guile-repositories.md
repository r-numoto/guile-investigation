...
- **用途**: **JSWF Player** の本体実装。Flash（SWF）コンテンツを JavaScript で動作させるプレイヤーサービス。広告フロントエンド（`guile-frontend-*`）から利用されるプレイヤーライブラリ。Flash（SWF）資産と ogv.js（HTML5 動画）を含む。`jswf` という名称は **JavaScript + SWF** の略。
