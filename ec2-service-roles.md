...
| ドメイン | 内容 |
|---|---|
| `mypage.guile.jp` | 広告主向けマイページ（PHP フロントエンド）**【本番】** |
| `api.guile.jp` | GUILE API（PHP バックエンド）**【本番】** |
| `stg.guile.jp` | ステージング フロントエンド（`SMV_ENV=stg`）**【ステージング】** |
| `stg-api.guile.jp` | ステージング API（`SMV_ENV=stg`）**【ステージング】** |
| `backend.jswfplayer.jp` | `mypage.guile.jp` へ 301 リダイレクト（jswf 廃止後の移行済み） |

> ⚠️ **本番とステージングを1台で兼用**。`SMV_ENV` 環境変数で切り替えている。
...
| 項目 | 内容 |
|---|---|
| ドメイン | `send-test-guile.sonicmoov.com` |
| OS | Amazon Linux AMI 2015.09 |
| ミドルウェア | nginx + td-agent + Redis + Python（CloudWatch Logs） |
| 役割 | **ステージング admovie-server**（名前に反して GUILE フロントエンドではない） |

**動作内容**
- nginx で `send-test-guile.sonicmoov.com`（ステージング admovie 配信エンドポイント）を提供
- php-fpm なし（PHP アプリは動いていない）
- td-agent でステージング admovie のアクセスログを収集・集計
- Redis でインプレッション/ビュー/スタート数をカウント（本番 collector1 の代替）
- Python = AWS CloudWatch Logs エージェント

> ⚠️ **命名の注意**: インスタンス名は "staging" だが、GUILE のステージング環境（`stg.guile.jp`）は admin インスタンスが担っている。
